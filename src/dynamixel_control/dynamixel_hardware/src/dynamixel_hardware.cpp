...
CallbackReturn DynamixelHardware::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  set_control_mode(ControlMode::Joint, true);
  set_joint_params();
  enable_torque(true);
  ...
  // Add Code
   const ControlItem * goal_joint =
    dynamixel_workbench_.getItemInfo(joint_ids_[0], kGoalPositionItem);
  if (goal_joint == nullptr) {
    return CallbackReturn::ERROR;
  }
  // Add Code
  ...
  const rclcpp::Duration & /* period */)
{
  ...
  //     kPresentPositionVelocityCurrentIndex, joint_ids_.data(), ids.size(),
  ...
  for (size_t i = 0; i < ids.size(); i++) {
      int32_t position, velocity, current;
      if (!dynamixel_workbench_.readRegister(ids[i], "Present_Position", &position)) {
          RCLCPP_ERROR(rclcpp::get_logger(kDynamixelHardware), "Erro ao ler posição do motor %d", ids[i]);
      }
      // if (!dynamixel_workbench_.readRegister(ids[i], "Present_Velocity", &velocity)) {
      //   RCLCPP_ERROR(rclcpp::get_logger(kDynamixelHardware), "Erro ao ler velocidade do motor %d", ids[i]);
        if (!dynamixel_workbench_.readRegister(ids[i], "Present_Speed", &velocity)) {
          RCLCPP_ERROR(rclcpp::get_logger(kDynamixelHardware), "Erro ao ler velocidade do motor %d", ids[i]);
      }
      if (!dynamixel_workbench_.readRegister(ids[i], "Present_Load", &current)) {
          RCLCPP_ERROR(rclcpp::get_logger(kDynamixelHardware), "Erro ao ler corrente do motor %d", ids[i]);
      }
  ...
  const rclcpp::Duration & /* period */)
{
      ...
      }))
  {
    ...
    set_control_mode(ControlMode::Joint);
    if (mode_changed_) {
      set_joint_params();
    }
    set_joint_positions();
...
return_type DynamixelHardware::set_control_mode(const ControlMode & mode, const bool force_set)
{
  ...
  if (mode == ControlMode::Joint && (force_set || control_mode_ != ControlMode::Joint)) {
    bool torque_enabled = torque_enabled_;
    if (torque_enabled) {
      enable_torque(false);
    }
  ...
  if (control_mode_ != ControlMode::Velocity && control_mode_ != ControlMode::Joint) {
    RCLCPP_FATAL(
...
CallbackReturn DynamixelHardware::set_joint_positions()
{
  ...
  if (!dynamixel_workbench_.syncWrite(
      ...
      kGoalPositionIndex, joint_ids_.data(), joint_ids_.size(), commands.data(), 1, &log))
  {
    RCLCPP_ERROR(rclcpp::get_logger(kDynamixelHardware), "%s", log);
  }
  return CallbackReturn::SUCCESS;
...
CallbackReturn DynamixelHardware::set_joint_velocities()
{
  ...
  if (!dynamixel_workbench_.syncWrite(
      ...
      // kGoalVelocityIndex, joint_ids_.data(), ids.size(), commands.data(), 1, &log))
  {
    RCLCPP_ERROR(rclcpp::get_logger(kDynamixelHardware), "%s", log);
  }
  return CallbackReturn::SUCCESS;
